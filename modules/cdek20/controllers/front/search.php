 ...
 */
class Cdek20SearchModuleFrontController extends ModuleFrontController
{
    public function init()
    {
        parent::init();

        if (Tools::getValue('type') == 'city') {
            $term = Tools::getValue('term', Tools::getValue('q'));

            $res = Db::getInstance()->executeS(
                'SELECT CONCAT(`city`, " ", `region`) as label, `code` as value FROM `' . _DB_PREFIX_ . 'cdek_city_lang` WHERE `city` LIKE "' . pSQL(
                    $term
                ) . '%" GROUP BY code ORDER BY city ASC LIMIT 10'
            );
