...
function ISDEKWidjet(params) {
	...
	if (!params.servicepath) {
		params.servicepath = params.path + 'widget';
	}
	...
	var widjet = new ipolWidjetController({
		...
		stages: {
			...
			'mainInit': {
				...
				function: function () {
					this.service.loadTag(this.options.get('path') + "js/widjet/ipjq.js", 'script', loaders.onIPJQLoad);
					var yalang = (this.options.get('lang') == 'rus') ? 'ru_RU' : 'en_GB';
					this.service.loadTag("https://api-maps.yandex.ru/2.1.66/?"+(widget_settings.map_api_key ? 'apikey='+widget_settings.map_api_key+'&' : '')+"lang="+yalang, 'script', loaders.onYmapsLoad);
					this.service.loadTag(this.options.get('path') + 'css/widjet/style.css', 'link', loaders.onStylesLoad);
			...
			'afterJquery': {
				...
				function: function () {
					...
					if (typeof PVZ == 'object') {
						DATA.parsePVZFile({pvz:PVZ});
					} else {
						ipjq.getJSON(
					...
					if (typeof widget_lang != 'undefined') {
						LANG.write({LANG:widget_lang});
					} else {
						ipjq.getJSON(
							widjet.options.get('servicepath'),
			...
			'dataLoaded': {
				...
				function: function () {
					loaders.onDataLoad();
					var cityFrom = widjet.options.get('cityFrom');
					if (isNaN(cityFrom)) {
						if (cityFrom) {
							ipjq.getJSON(
								widjet.options.get('servicepath'),
								{isdek_action: 'getCity', cityFrom},
								function (data) {
	...
	widjet.showLoader = function () {
		if (widjet.loaderHided) {
			widjet.loaderHided = false;
			ipjq(IDS.get('PRELOADER')).fadeIn(300);
			ipjq(IDS.get('PRELOADER')).css({position:'relative'});
			ipjq(IDS.get('cdek_widget_cnt')).find('.CDEK-widget__search, .CDEK-widget__sidebar, .CDEK-widget__logo').addClass('CDEK-widget__inaccessible');
		}
	}
	...
	var CALCULATION = {
		...
		request: function (type, timestamp) {
			...
			if (DATA.city.current)
				if (typeof cdek_calc[data.type] == 'object') {
					CALCULATION.onCalc(cdek_calc[data.type]);
					delete cdek_calc[data.type];
				} else {
					ipjq.getJSON(
						widjet.options.get('servicepath'),
	...
	var LANG = {
		...
		write: function (data) {
			// upload templates after the language
			if (typeof(data.LANG) === 'undefined') {
				...
				if (typeof widget_templates != 'undefined') {
					HTML.save(widget_templates);
				} else {
					ipjq.getJSON(
	...
	var template = {
		...
		controller: {
			...
			redirectNewCity: function (city) {
				var delivery_option = 0;
				var delivery_option_el = window.parent.$(window.parent.cdek.option_selector);
				if (delivery_option_el.length > 1) {
					delivery_option = window.parent.$(window.parent.cdek.option_selector + ':checked').val();
				} else {
					delivery_option = delivery_option_el.val();
				}
				if (false && window.id_order) { // TODO ?????
					$.ajax({
						url: widjet.options.get('widgetpath'),
						dataType: 'json',
						method: 'post',
						data: {type: 'pickup', city: city, city_change: 1, id_order: window.id_order},
						success: function (r) {
							if (r.hasError) {
								alert('Error');
							} else {
								template.controller.setInfoBlock(r.city, r.address);
							}
						}
					});
				} else if (window.parent.postcode_shortcut) {
					//void
				} else {
					widjet.showLoader();
					var id_order = window.parent.id_order;
					$.ajax({
						url: widjet.options.get('widgetpath'),
						async: false,
						dataType: 'json',
						method: 'post',
						data: {ajax: 1, city: city, action: 'city_change', delivery_option: delivery_option, id_order: id_order},
						success: function (r) {
							if (r.hasError) {
								alert(window.parent.cdek.trans('This delivery method is not available for this city'));
								return false;
							} else {
								if (id_order) {
									window.parent.$('.row.cdek_widget').html($(r.widget))
			...
			selectCity: function (city) {
				// for recalculate order page
				if (!this.redirectNewCity(city)) {
			...
			setInfoBlock: function (city, address) {
				ipjq('#info_city').html(city);
				ipjq('#info_address').html(address);
			},
			choosePVZ: function (id) {
				...
				ipjq(IDS.get('choose_button')).fadeOut();
				...
				if (typeof pvzList === 'object') {
					...
					for (var i in pvzList) {
						...
						pvzList[i].placeMark = new ymaps.Placemark([pvzList[i].cY, pvzList[i].cX], {}, {
							...
							iconImageHref: widjet.options.get('path') + 'img/widjet/sdekNActive.png',
							iconImageSize: [40, 43],
						...
						pvzList[i].placeMark.events.add(['mouseenter'], function (metka) {
							...
							metka.get('target').options.set({iconImageHref: widjet.options.get('path') + "img/widjet/sdekActive.png"});
						});

						pvzList[i].placeMark.events.add(['mouseleave'], function (metka) {
							if (template.ui.currentmark != metka.get('target')) {
								metka.get('target').listItem.removeClass('CDEK-widget__panel-list__item_active');
								metka.get('target').options.set({iconImageHref: widjet.options.get('path') + "img/widjet/sdekNActive.png"});
							}
						});
			...
			blinkPVZ: function (event) {
				...
				if (event.data.link.readyToBlink) {
					...
					if (event.data.ifOn) {
						...
						cityPvz[event.data.id].placeMark.options.set({iconImageHref: widjet.options.get('path') + "img/widjet/sdekActive.png"});
					} else {
						cityPvz[event.data.id].placeMark.options.set({iconImageHref: widjet.options.get('path') + "img/widjet/sdekNActive.png"});
	...
	widjet.sdekWidgetEvents = function () {
		...
		ipjq(IDS.get('cdek_widget_cnt')).on('click', '.CDEK-widget__sidebar-button', {widjet: widjet}, function () {
            ...
            $('.CDEK-widget__sidebar-burger').trigger( "click" );
        });
