...
class ToolsModuleSK
{
            ...
            if (array_key_exists($field, $definition_fields)) {
                ...
                if (array_key_exists('lang', $definition_fields[$field]) && $definition_fields[$field]['lang']) {
                    foreach ($languages as $lang) {
                        ...
                        if (!empty($object_field[$lang['id_lang']]) && forward_static_call_array(
                            array('Validate', $definition_fields[$field]['validate']),
                            array($object_field[$lang['id_lang']])
                        ) && array_key_exists('size', $definition_fields[$field]) && Tools::strlen(
                            $object_field[$lang['id_lang']]
    ...
    public static function registerSmartyFunctions()
    {
        ...
        if (!array_key_exists('get_image_lang_cdek', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'get_image_lang_cdek', array(__CLASS__, 'getImageLangCdek'));
        }
    ...
    public static function getImageLangCdek($smarty)
    {
        if (_PS_VERSION_ < 1.5) {
