...
class HelperDbCMP
{
    ...
    {
        if ($field_type == ObjectModel::TYPE_STRING || $field_type == ObjectModel::TYPE_HTML
    ...
    public function getSQLDefaultVal($field, $validate, $required = false)
    {
        ...
        switch ($validate) {
            ...
            case 'isBool':
                return pSQL($default_val) . ' DEFAULT "' . (isset($this->instance->{$field}) && $this->instance->{$field}
                        ? pSQL($this->instance->{$field}) : '0') . '"';
            case 'isDateFormat':
                return pSQL($default_val) . ' DEFAULT "'
                    . (isset($this->instance->{$field}) && $this->instance->{$field} ? pSQL(
                        $this->instance->{$field}
                    ) : '0000-00-00 00:00:00') . '"';
            default:
                return ($required
                    ? $default_val . (isset($this->instance->{$field}) && $this->instance->{$field} ? ' DEFAULT "' . pSQL(
                            $this->instance->{$field}
                        ) . '"' : '') : (isset($this->instance->{$field}) && $this->instance->{$field} ? ' DEFAULT "' . pSQL(
                            $this->instance->{$field}
                        ) . '"' : 'DEFAULT NULL'));
        }
    }
    ...
    public function updateObjectField($field, $id, $value)
    {
        ...
        $multi_lang_shop = (array_key_exists(
            'multilang_shop',
            $definition
        ...
        if (!$multi_shop_active || ($lang && $multi_lang_shop) || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            if ($multi_shop_active && $lang && $multi_lang_shop) {
                $sql .= ' AND `id_shop` IN(' . (count($ids_shop) ? implode(
                        ',',
    ...
    {
        $fields = [];
        $fields[$this->definition['primary']] = ['type' => ObjectModel::TYPE_INT, 'validate' => 'isUnsignedInt'];
