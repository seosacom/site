...
class ToolsModuleCMP
{
     ...
     * @return void
     */
    public static function createTab($module_name, $class_name, $parent = null, $name = null)
    {
        if (!is_array($name)) {
            $name = array('en' => $name);
    ...
    public static function getTemplateDir($path)
    {
        if (Tools::file_exists_cache(
            _PS_THEME_DIR_.'modules/'.self::getModNameForPath(__FILE__).'/views/templates/'.$path
        )
        ) {
    ...
    public static function registerSmartyFunctions()
    {
        ...
        if (!array_key_exists('get_image_lang_payment', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'get_image_lang_payment', array(__CLASS__, 'getImageLangPayment'));
        }
        if (!array_key_exists('renderTemplate', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'renderTemplate', array(__CLASS__, 'renderTemplate'));
        }
        if (!array_key_exists('versionCompare', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'versionCompare', array(__CLASS__, 'versionCompare'));
        }
        ...
        if (!array_key_exists('roundPrice', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'roundPrice', array(__CLASS__, 'roundPrice'));
        }
        if (class_exists('TransModCMP')) {
            if (!array_key_exists('ld', $smarty->registered_plugins['modifier'])) {
                smartyRegisterFunction($smarty, 'modifier', 'ld', array(TransModCMP::getInstance(), 'ld'));
            }
    ...
    {
        $price = Tools::displayPriceSmarty($price, Context::getContext()->smarty);
        $price = preg_replace('|,00|', '', $price);
        if (preg_match('|,\d0|', $price)) {
            $price[strpos($price, ',') + 2] = '';
        }
        
        return $price;
    }
    
    public static function getImageLangPayment($smarty)
    {
        $path = $smarty['path'];
        ...
        if (function_exists('error_get_last')) {
            ...
            if ($error && $error['type'] === E_ERROR) {
                ...
                if (preg_match($memory_regex, $message, $matches)) {
                    ...
                    $message .= ')';
                    LoggerCMP::getInstance()->error($message);
                    LoggerCMP::getInstance()->error(
                ...
                die(
                    json_encode(
                        array(
                            'hasError' => LoggerCMP::getInstance()->hasError(),
                            'log'      => LoggerCMP::getInstance()->getMessages(),
                        )
                    )
                );
            }
    ...
    public static function createAjaxApiCall($class)
    {
        ...
        if (method_exists($class, $call_method)) {
            try {
                ...
                LoggerCMP::getInstance()->exception($e);
                die(
                    json_encode(
                        array(
                            'hasError' => LoggerCMP::getInstance()->hasError(),
    ...
    public static function getModuleTabAdminLink()
    {
        ...
        return Context::getContext()->link->getAdminLink('AdminModules', true)
            .'&configure='.self::getModNameForPath(
                __FILE__
